...
## Calculating Seasonality by Industry in Philadelphia
...
ts.index <- ts.join.w %>%
  select(Industry, date, "Seasonally" = `Seasonally Adjusted`, "nSeasonally" = `Non-Seasonally Adjusted`) %>%
...
for (i in ind.list) {
  ...
  temp.plot <- industry %>%
    ggplot(aes(x = date, y= `Total Employment`, group=measure, color = measure)) +
    geom_line(size = 2) +
  ...
  ggsave(temp.plot, file = paste0(i, " - Employment Trend.png"), device = "png", height = 5, width = 11, units = "in")
